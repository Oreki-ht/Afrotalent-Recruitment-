...
public class Sort1GUI extends JFrame {
    private JTable table;
    public Sort1GUI() {
        try {
            Connection con = DriverManager.getConnection("jdbc:postgresql://localhost:1412/afrotalent", "postgres", "conanendou23");
            Statement stmt = con.createStatement(); 
            ResultSet rs = stmt.executeQuery("SELECT * FROM intake WHERE tday = '" + SortGUI.sortedday + "' AND tmonth = '" + SortGUI.sortedmonth + "' AND tyear = '" + SortGUI.sortedyear + "'");
